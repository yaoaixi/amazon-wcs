...
Month = {
    ...
    'com.mx': {'month': ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio',
                         'Julio', 'Agosto', 'Septiembre', 'Octubre', 'noviembre', 'Diciembre'],
               'xpath_text': 'Producto en Amazon.com.mx desde'},
}
...
class ParseProduct:
    ...
    def _handle_str_xpath(self, *args):
        result = ''
        for item in args:
            _result = ''.join(it.strip().replace('\n', ' ') for it in item)
            if _result:
                result = _result
                break
        return result
    ...
    def get_title(self):
        ...
        title_7 = self.sel.xpath('//span[@id="btAsinTitle"]/span/text()')
        title_8 = self.sel.xpath('//*[@id="product-title"]/h1[1]/text()')
        return self._handle_str_xpath(title_1, title_2, title_3, title_4, title_5, title_6, title_7, title_8)
    ...
    def get_asin(self):
        ...
        asin = self._handle_str_xpath(asin_1, asin_2, asin_3, asin_4)
        if len(asin) > 15:
                asin = ''
        return asin
    ...
    def get_brand(self):
        ...
        brand_5 = self.sel.xpath('//div[contains(@class, "a-box-inner")]/a/text()')
        if brand_3:
            brand_3 = brand_3[0].xpath('./../td/text()')
        return self._handle_str_xpath(brand_1, brand_2, brand_3, brand_4, brand_5)
    ...
    def get_original_price(self):
        ...
        if original_price:
            original_price = ''.join(original_price).strip().replace(sign, '').replace(currency, '').replace(' ', '').replace(
                '\xa0', '')
            if currency == 'EUR':
    ...
    def get_price_maxprice(self):
        ...
        price_8 = self.sel.xpath('//div[@id="product-price"]//span[contains(@class,"a-color-price")]/text()')
        if price_1 and price_1[0].strip():
            ...
            price = price_5[0].strip().replace(sign, '').replace(currency, '').replace(' ', '').replace('\xa0', '').replace(',', '')
            max_price = 0
        elif price_6:
            price = price_6[0].strip().replace(sign, '').replace(currency, '').replace(' ', '').replace('\xa0', '').replace(',', '')
            max_price = 0
        elif price_7:
            price = price_7[0].strip().replace(sign, '').replace(currency, '').replace(' ', '').replace('\xa0', '').replace(',', '')
            max_price = 0
        elif price_8:
            price = price_8[0].strip().replace(sign, '').replace(currency, '').replace(' ', '').replace('\xa0', '').replace(',', '')
            max_price = 0
        else:
            price = 0
            max_price = 0
    ...
    def get_grade_count(self):
        ...
        grade_count_4 = self.sel.xpath('//*[contains(@class, "a-icon-star")]//span[contains(@class, "a-icon-alt")]/text()')
        if grade_count_1:
            ...
            grade_count = grade_count_4[0].strip().split(' ')[0]
        else:
            grade_count = 0
        try:
    ...
    def get_review_count(self):
        ...
        review_count_3 = self.sel.xpath('//a[contains(text(), "customer reviews")]/text()')
        if review_count_1:
    ...
    def get_attribute(self):
        ...
        color_1 = self.sel.xpath('//div[@class="a-row"]//ul[contains(@class, "a-unordered")]/li//span[@data-swatch-color]/@data-swatch-color')
        if color:
            if color[0].xpath(
            ...
            attribute['color'] = [item.strip() for item in color_1]

        style = self.sel.xpath('//div[@id="variation_style_name"]')
        style_1 = self.sel.xpath('//*[contains(@id, "variation-selector-dropdown-page-style")]/option/text()')
        if style:
            ...
            attribute['style'] = [st.strip() for st in style_1]

        trim = self.sel.xpath('//*[contains(@id, "variation-selector-dropdown-page-trim")]/option/text()')
        if trim:
            attribute['trim'] = [tr.strip() for tr in trim]
    ...
    def get_main_image(self):
        ...
        image_url_6 = self.sel.xpath('//div[@id="image_block_main_image_container"]//li//img/@src')
        if image_url_1:
            ...
            main_image_url = image_url_6[0].strip()
        else:
            main_image_url = ''
    ...
    def get_extra_images(self):
        ...
        extra_image_urls_4 = self.sel.xpath('//div[@id="image_block_main_image_container"]//li//img/@src')
        ...
        elif len(extra_image_urls_4) > 1:
            _extra_image_urls = extra_image_urls_4[1:]
        else:
            _extra_image_urls = ''
    ...
    def get_description(self):
        ...
        description_3 = self.sel.xpath('//div[contains(@class, "a-section a-padding-mini")]//text()')
        return self._handle_str_xpath(description_1, description_2, description_3)
    ...
    def get_generations_time(self):
        ...
        try:
            ...
            if self.suffix in ('com', 'co.uk', 'com.au', 'ca', 'in'):  # 英语国家两种匹配规则
            ...
            print('get_generations_time raise a exc: {}'.format(e))
    ...
    def get_reserve_1(self):
        ...
        for h in _histogram_review_count[:5]:
            ...
            if percent_1:
                percent = ''.join([pe.strip() for pe in percent_1])
            elif percent_2:
                percent = percent_2[0].strip()
            else:
                percent = '0%'
            histogram_review_count[star] = percent
        if not histogram_review_count:
            histogram_review_count = {"5": "0%", "4": "0%", "3": "0%", "2": "0%", "1": "0%"}
        return json.dumps(histogram_review_count, ensure_ascii=False)
    ...
    def get_reserve_4(self):
        ...
        return self._handle_str_xpath(reserve_field_4_1, reserve_field_4_2).replace(':', '')
